...
transient public int prodotti[];
	...
	//@ ensures \result==false <==> (\old(prodotti[productIndex])+addQuantity > 100);
	//@ ensures \result==true <==> (\old(prodotti[productIndex])+addQuantity)<=100;
	...
	//@ ensures \result==true ==> (\forall int i; i>=0 && i<prodotti.length; \old(prodotti[i])==100);
	//@ ensures \result==false ==> (\exists int i; i>=0 && i<prodotti.length; \old(prodotti[i])<100);
	//@ diverges true;
